...
void AbstractExercise::setFromTop(const bool fromTop)
{
	m_fromTop = fromTop;
}

const bool AbstractExercise::fromTop() const
{
	return m_fromTop;
}
...
bool AbstractExercise::onKeyPressed(QKeyEvent * keyEvent)
{
	...
	if (keyEvent->key() == Qt::Key_C && !keyEvent->modifiers())
	{
		setFromTop(!m_fromTop);
