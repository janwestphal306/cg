...
std::array<QVector3D, 12> Icosahedron::vertices()
{
    static const float t = (1.f + qSqrt(5.f)) * 0.5f; // 2.118
    static const float i = 1 / qSqrt(t * t + 1.f);  // 0.427
    static const float a = t * i;                     // 0.904

    // icosahedron vertices (normalized) form three orthogonal golden rectangles
    // http://en.wikipedia.org/wiki/Icosahedron#Cartesian_coordinates

    return std::array<QVector3D, 12>{{
            QVector3D(-i, a, 0)
                , QVector3D(i, a, 0)
                , QVector3D(-i, -a, 0)
                , QVector3D(i, -a, 0)
...
void Icosahedron::initialize(QOpenGLShaderProgram & program)
{
    ...
    std::vector<QVector3D> vertices(v.begin(), v.end());
    std::vector<Face> indices(i.begin(), i.end());

    refine(vertices, indices, static_cast<char>(qBound(0, m_iterations, 8)));
    ...
    m_vertices.allocate(vertices.data(), vertices.size() * sizeof(std::vector<QVector3D>::value_type));
...
,   std::unordered_map<uint, GLushort> & cache)
{
    ...
    points.push_back(((points[a] + points[b]) * .5f).normalized());
